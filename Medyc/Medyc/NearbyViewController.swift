...
class NearbyViewController: UIViewController, CLLocationManagerDelegate, UITableViewDelegate, UITableViewDataSource, MKMapViewDelegate {
    ...
    var samplelocationsTable = UITableView()
    ...
    var imagesReference = ["General Practitioner": #imageLiteral(resourceName: "generalPractitioner"), "Dentist": #imageLiteral(resourceName: "dentist"), "Optometrist": #imageLiteral(resourceName: "optometrist"), "Gynecologist": #imageLiteral(resourceName: "gynocologist"), "Psychologist": #imageLiteral(resourceName: "psychologist"), "Cardiologist": #imageLiteral(resourceName: "cardiologist"), "Dermatologist": #imageLiteral(resourceName: "dermatologist"), "Physiotherapist": #imageLiteral(resourceName: "physiotherapist"), "Podiatrist": #imageLiteral(resourceName: "podiatrist"), "Psychiatrist": #imageLiteral(resourceName: "psychiatrist")]
    ...
    override func viewDidLoad() {
        ...
        locationsTable.register(UITableViewCell.self, forCellReuseIdentifier: "cell")
        //locationsTable.backgroundColor = .red
    ...
    func swipe(sender: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            //} else {
            if distance > locationsExpandeGuide.frame.minY {
            ...
            if (self.locationTableBackground.frame.minY - self.guideView.frame.minY) > 20 {
                ...
                UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0, options: .curveEaseOut, animations: {
                    ...
                    if self.guideView == self.locationsExpandeGuide {
                        ...
                        self.locationsTable.frame.size.height = self.locationsNormalGuide.frame.height - self.favouritesButton.frame.height
                    } else {
                        self.locationTableBackground.frame.origin.y = self.locationsMinimisedGuide.frame.minY
                        self.locationsTable.frame.size.height = self.locationsMinimisedGuide.frame.height - self.favouritesButton.frame.height
                    }
                ...
                UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: .curveEaseOut, animations: {
                    ...
                    if self.guideView == self.locationsMinimisedGuide {
                        ...
                        self.locationsTable.frame.size.height = self.locationsExpandeGuide.frame.height - self.favouritesButton.frame.height + self.locationTableBackground.layer.cornerRadius
                    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if tableView == locationsTable {
            ...
            categoryImg.image = imagesReference[currentCategory]
            
            if search.isSearching {
                search.cancel()
            }
            self.performSearch(region: savedRegion, searchTerms: [""], radius: Double(savedRadius))
            //add refresh of map
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        anView?.image = #imageLiteral(resourceName: "locationPin")
        anView?.frame.size = CGSize(width: 20, height: 25)
  ...
  /*
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        let annotationView = MKAnnotationView(annotation: annotation, reuseIdentifier: "annot")
        if annotation.isEqual(mapView.userLocation) {
            //print(annotation.)
            return nil
        }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.changeSearchView.bringSubview(toFront: categoryImg)
        ...
        if !orientationChanged {
            ...
            locationTableBackground.frame.size.height = self.locationsExpandeGuide.frame.size.height  + self.locationTableBackground.layer.cornerRadius
        ...
        //locationsTable.isHidden = true
            ...
            favouritesButton.frame.size = locationsMinimisedGuide.frame.size//CGSize(width: locationTableBackground.frame.width, height: 60)
            favouritesButton.frame.origin = .zero//.center = CGPoint(x: self.locationTableBackground.frame.width/2, y: self.favouritesButton.frame.height/2)
        
            favouritesIcon.frame.size = CGSize(width: favouritesButton.frame.height/2, height: favouritesButton.frame.height/2)
            favouritesIcon.center = CGPoint(x: favouritesButton.frame.height/2 + favouritesIcon.frame.width/2, y: self.favouritesButton.center.y)
        ...
        favouritesLabel.font = UIFont.systemFont(ofSize: 19, weight: UIFontWeightMedium)
            ...
            locationsTable.frame.size.height = self.locationsMinimisedGuide.frame.height - self.favouritesButton.frame.height
        
            locationsTable.frame.size = self.locationsExpandeGuide.frame.size
            locationsTable.frame = CGRect(origin: CGPoint(x: 0, y: locationsMinimisedGuide.frame.height), size: locationsNormalGuide.frame.size)
        orientationChanged = true
        }
        
        self.changeSearchView.bringSubview(toFront: categoryImg)
        self.categoryImg.alpha = 1
        self.categoryImg.isHidden = false
        ...
        if manager.location != nil {
            CLGeocoder().reverseGeocodeLocation(manager.location!, completionHandler: {(placemarks, error) -> Void in
                ...
                if placemarks!.count != 0 {
                    ...
                    self.performSearch(region: region, searchTerms: ["General Practitioner"], radius: 1000)
                } else {
    ...
    func performSearch(region: MKCoordinateRegion, searchTerms: [String], radius: Double)  {
        
        
        self.clinicsFound.removeAll()
        self.mapView.removeAnnotations(self.mapView.annotations)
        self.mapItemsFound.removeAll()
        ...
        if !activityIndicator.isAnimating {
            ...
            request.naturalLanguageQuery = currentCategory
            ...
            search.start { response, error in
                ...
                //if index == searchTerms.count - 1 {
                    ...
                    if self.mapItemsFound.count >= 3 {
                        ...
                        self.performSearch(region: region, searchTerms: searchTerms, radius: radius + 2000)
                    }
    ...
    func curateSearchTerms(region: MKCoordinateRegion, placemark: CLPlacemark) {
        ...
        self.performSearch(region: region, searchTerms: unique, radius: 1000)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == locationsTable {
            ...
            cell.textLabel?.text = clinicsFound[indexPath.row].name
            cell.textLabel?.font = UIFont.systemFont(ofSize: (cell.textLabel?.font.pointSize)! + 2.0, weight: UIFontWeightMedium)
            ...
            if let suburb = mapItemsFound[indexPath.row].placemark.locality {
                ...
                cell.detailTextLabel?.font = UIFont.systemFont(ofSize: (cell.detailTextLabel?.font.pointSize)! + 3.0, weight: UIFontWeightRegular)
            }
            ...
            for subview in cell.subviews {
                ...
                if let _:UIImageView = subview as? UIImageView {
                    subview.removeFromSuperview()
                }
            }
            ...
            let catIcon = UIImageView(frame: self.categoryImg.frame)
            cell.addSubview(catIcon)
            catIcon.image = imagesReference[categories[indexPath.row]]
            catIcon.center.y = cell.contentView.center.y
    ...
    @IBAction func changeCategory(_ sender: Any) {
        ...
        if selectingCategory {
            ...
            UIView.animate(withDuration: 0.33, animations: {
                ...
                self.categoryImage.transform = CGAffineTransform(rotationAngle: (-180.0 * CGFloat(Double.pi)) / 180.0)
                
                
                self.categoriesTable.frame = CGRect(x: self.categoriesTable.frame.minX, y: self.changeSearchView.frame.maxY, width: self.changeSearchView.frame.width, height: 164)
                self.listBackground.frame = CGRect(x: self.listBackground.frame.minX, y: self.changeSearchView.frame.maxY, width: self.changeSearchView.frame.width, height: 164)
                self.listShadow.frame = CGRect(x: self.listShadow.frame.minX, y: self.changeSearchView.frame.maxY + (self.changeSearchView.frame.height - self.buttonShadow.frame.height)/2 , width: self.buttonShadow.frame.width, height: 164 - (self.changeSearchView.frame.height - self.buttonShadow.frame.height))
    ...
    func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        ...
        favorite.backgroundColor = UIColor(netHex: 0xDE558C)
